...
import { fetchVotersbyphone, saveVoter,updateVoter } from '../Services/voterServices';
...
export function VoterEditForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
try {
    const result = await updateVoter(formData,params.phone);
    console.log(result);
} catch (error) {
    console.log(error);
}
  ...
  const  populateVoterstate= async ()=>{
    try {
       const result =  await fetchVotersbyphone(params.phone);
       setVoters(result.student);
    } catch (error) {
        console.log(error);
    }
  }
